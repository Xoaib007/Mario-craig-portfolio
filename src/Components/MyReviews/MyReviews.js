...
const MyReviews = () => {
    ...
    const { data: myReviews = [], refetch} = useQuery({
        queryKey: ['booking'],
        queryFn: () => fetch(`https://mario-craig-server.vercel.app/reviews/user/${user?.uid}`).then(res => res.json())
    })
                        ...
                        <div key={myReviews._id}>
                            ...
                            <form onSubmit={(e) => handleEditReview(e, myReviews?._id)} className="modal">
                                <div className="modal-box bg-black ">
                                    <label htmlFor={myReviews._id} className="btn btn-sm btn-circle absolute right-2 top-2">✕</label>
                                    ...
                                    <div className="modal-action">
                                        <label htmlFor={myReviews._id} className="btn btn-sm relative right-4 bottom-4 bg-orange-400">Cancel</label>
                                        <button type='submit' className="btn btn-sm relative right-4 bottom-4 bg-orange-400">Done</button>
                                    </div>
                                </div>
                            </form>
